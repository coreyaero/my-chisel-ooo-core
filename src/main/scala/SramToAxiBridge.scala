...
class SramToAxiBridge(implicit p: CacheConfig) extends Module {
    ...
    when(ar_fire) {
        ...
        // 确保最高位为0（会被Crossbar无损截断），第2位是Cache标志(1为DCache, 0为ICache)，第1~0位是真正的MSHR编号
        when(dcache_read_safe) {
            ar_grant_id := Cat(0.U(1.W), 1.U(1.W), io.data_cache.rd_id(1, 0))
            ar_addr_reg := io.data_cache.rd_addr
            ar_size_reg := io.data_cache.rd_type
        } .otherwise {
            ar_grant_id := Cat(0.U(2.W), io.inst_cache.rd_id(1, 0)) // 0.U(2.W) 代表最高两位都是 0
    ...
    // ★ 绝对信任 AXI 传回的 3 位真实 RID
    val real_rid = io.axi.rid 

    // 数据端 (DCache) 响应：第 2 位为 1
    val is_data_ret = io.axi.rvalid && (real_rid(2) === 1.U)
    io.data_cache.ret_valid := is_data_ret
    io.data_cache.ret_id    := real_rid(1, 0) // 完美还原 2 位 MSHR ID！
    io.data_cache.ret_last  := io.axi.rlast
    io.data_cache.ret_data  := io.axi.rdata

    // 指令端 (ICache) 响应：第 2 位为 0
    val is_inst_ret = io.axi.rvalid && (real_rid(2) === 0.U)
    io.inst_cache.ret_valid := is_inst_ret
    io.inst_cache.ret_id    := real_rid(1, 0) // 完美还原 2 位 MSHR ID！
