error id: file://<WORKSPACE>/src/main/scala/Exec.scala:
file://<WORKSPACE>/src/main/scala/Exec.scala
empty definition using pc, found symbol in pc: 
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -chisel3/br_broadcasted.
	 -chisel3/br_broadcasted#
	 -chisel3/br_broadcasted().
	 -chisel3/util/br_broadcasted.
	 -chisel3/util/br_broadcasted#
	 -chisel3/util/br_broadcasted().
	 -br_broadcasted.
	 -br_broadcasted#
	 -br_broadcasted().
	 -scala/Predef.br_broadcasted.
	 -scala/Predef.br_broadcasted#
	 -scala/Predef.br_broadcasted().
offset: 27351
...
class AguUnit extends Module {
    ...
    //==========================================
    // EX1 (Comb) & EX2 Pipeline
    //==========================================
    // ★ 核心拆除：EX1 化身极限组合逻辑，完美填补之前被我们加 RegNext 吃掉的那 1 拍！
    val ex1_active = io.in.valid && !is_killed(io.in.bits.branch_mask)
    val ex1_data   = WireDefault(io.in.bits)
    ...
    when(io.flush) {
        ...
        when(ex2_ready) {
            ...
            when(is_killed(ex2_data.data.branch_mask)) {
                ex2_valid := false.B
            }
        }
    }
    ...
    //==========================================
    // EX1 Execution (Now Combinational!)
    //==========================================
    ...
    next_ex2.data.branch_mask   := ex1_data.branch_mask // 这里不用再 & clear_mask，因为上面已经被清洗过了
    next_ex2.va                 := ex1_va
...
class AluSimpleUnit extends Module {
    ...
    // Pipeline
    //==========================================
    val active = io.in.valid && !is_killed(io.in.bits.branch_mask)
    io.in.ready  := io.out.ready
    io.out.valid := active

    val data_wire = WireDefault(io.in.bits)
    data_wire.branch_mask := io.in.bits.branch_mask & clear_mask

    //==========================================
    // ALU
    //==========================================
    val alu_src1 = Mux(data_wire.src1IsPC, data_wire.pc, data_wire.src1_value)
    val alu_src2 = Mux(data_wire.src2IsImm, data_wire.imm, Mux(data_wire.src2IsFour, 4.U, data_wire.src2_value))

    val alu = Module(new ALU())
    alu.io.aluOp := data_wire.aluOp
    alu.io.src1  := alu_src1
    alu.io.src2  := alu_src2

    val out_data = WireDefault(data_wire) 
...
class AluUnit extends Module {
    ...
    when(io.flush) {
        ...
        br_broadcasted := false.B
    } .elsewhen(io.br_resolve.valid) {
        br_broadcasted := true.B
    }br@@_broadcasted := true.B
    }

    val data_wire = WireDefault(io.in.bits)
    data_wire.branch_mask := io.in.bits.branch_mask & clear_mask

    val src1_val = data_wire.src1_value
    val src2_val = data_wire.src2_value
    ...
    val branch_actual_taken = MuxLookup(data_wire.brType, false.B)(Seq(
    ...
    val branch_base_pc = Mux(data_wire.brType === BrType.JIRL, src1_val, data_wire.pc)
    val calc_target_pc = branch_base_pc + data_wire.imm

    val op = data_wire.inst(31, 26)
    val rd = data_wire.inst(4, 0)
    val rj = data_wire.inst(9, 5)
    ...
    val type_wrong = data_wire.is_branch && (data_wire.bpu_type =/= btype)
    val dir_wrong = (branch_actual_taken =/= data_wire.pred_taken)
    val addr_wrong = branch_actual_taken && (calc_target_pc =/= data_wire.pred_target)
    val mispredict = dir_wrong || addr_wrong || type_wrong

    val correct_next_pc = Mux(branch_actual_taken, calc_target_pc, data_wire.pc + 4.U)

    val do_br_resolve = active && data_wire.is_branch && !data_wire.hasException && !br_broadcasted
    io.br_resolve.valid      := do_br_resolve
    io.br_resolve.mispredict := mispredict
    io.br_resolve.tag        := data_wire.branch_tag
   ...
   io.bpu_update.valid := do_br_resolve
    io.bpu_update.bits.pc         := data_wire.pc
    io.bpu_update.bits.taken      := branch_actual_taken
    io.bpu_update.bits.target     := calc_target_pc
    io.bpu_update.bits.bpu_type   := btype
    io.bpu_update.bits.ghr        := data_wire.ghr
    io.bpu_update.bits.ras_tos    := data_wire.ras_tos
    io.bpu_update.bits.mispredict := mispredict
    io.bpu_update.bits.bimodal_pred := data_wire.bimodal_pred
    io.bpu_update.bits.gshare_pred  := data_wire.gshare_pred

    //==========================================
    // ALU
    //==========================================
    val alu_src1 = Mux(data_wire.src1IsPC, data_wire.pc, src1_val)
    val alu_src2 = Mux(data_wire.src2IsImm, data_wire.imm, Mux(data_wire.src2IsFour, 4.U, src2_val))
    ...
    val csr_mask = Mux(data_wire.src1_addr === 0.U, 0.U(32.W),
                   Mux(data_wire.src1_addr === 1.U, "hFFFFFFFF".U(32.W), src1_val))
    io.csr_raddr := data_wire.csrNum

    val final_ex_result = Mux(data_wire.rdtimel, io.timer_in(31, 0),
                          Mux(data_wire.rdtimeh, io.timer_in(63, 32),
                          Mux(data_wire.isCsr, io.csr_rdata, alu.io.res)))

    val out_data = WireDefault(data_wire) 
    out_data.ex_result := final_ex_result
    out_data.aux_data  := Mux(data_wire.isCsr, csr_mask, src2_val)
