error id: file://<WORKSPACE>/src/main/scala/AguUnit.scala:mycpu/AguUnit#early_is_mapped.
file://<WORKSPACE>/src/main/scala/AguUnit.scala
empty definition using pc, found symbol in pc: 
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -chisel3/early_is_mapped.
	 -chisel3/early_is_mapped#
	 -chisel3/early_is_mapped().
	 -chisel3/util/early_is_mapped.
	 -chisel3/util/early_is_mapped#
	 -chisel3/util/early_is_mapped().
	 -early_is_mapped.
	 -early_is_mapped#
	 -early_is_mapped().
	 -scala/Predef.early_is_mapped.
	 -scala/Predef.early_is_mapped#
	 -scala/Predef.early_is_mapped().
offset: 8487
...
class AguUnit extends Module {
    ...
    // ★ 4. 异常定案 (优先级仲裁)
    // ALE > MMU，且如果没被 mapped，MMU code 强制为 0
    val final_ecode = Mux(ale, "h09".U(6.W), Mux(need_tlb, raw_tlb_code, 0.U))
    
    // 只有在 ex2_active (真正存活) 时，新产生的异常才会被承认！
    val new_exc = ex2_active && (ale || (need_tlb && raw_tlb_code =/= 0.U))
    val final_has_exc = ex2_d.hasException || new_exc

    
    val mmu_code = Mux(early_is_mapped, raw_tlb_code, 0.U)
    val final_ecode = Mux(ale, "h09".U(6.W), mmu_code)
    val final_has_exc = ex2_d.hasException || ale || (early_is_ma@@pped && raw_tlb_code =/= 0.U)

    //==========================================
    // Output
    //==========================================
    val is_mem = (ex2_d.resFromMem || ex2_d.memWe || ex2_d.is_cacop) && ex2_active
    ...
    //==========================================
    // 异常终审
    val final_ecode = Mux(ale, "h09".U(6.W), Mux(need_tlb, tlb_ecode, 0.U))
    val new_exc = ex2_active && (ale || (need_tlb && tlb_ecode =/= 0.U))
    val final_has_exc = ex2_d.hasException || new_exc
    ...
    io.to_lsq.bits.paddr    := Cat(Mux(cacop_is_index, va(31,12), pa(31,12)), va(11, 0))
    ...
    val stMaskW = "b1111".U(4.W)
    val base_wstrb = Mux(isWord, stMaskW, Mux(isHalf, stMaskH, stMaskB))
    io.to_lsq.bits.wstrb    := Mux(ex2_d.memWe && ex2_active && !io.flush && !final_has_exc, base_wstrb, 0.U(4.W))
    
    io.to_lsq.bits.has_exc  := final_has_exc
    io.to_lsq.bits.ecode    := final_ecode
    ...
    out_data.hasException := final_has_exc
    out_data.ecode        := Mux(ex2_d.hasException, ex2_d.ecode, final_ecode)
    out_data.is_cacop     := Mux(ex2_active, ex2_d.is_cacop, false.B)
    out_data.cacop_op     := Mux(ex2_active, ex2_d.cacop_op, 0.U(5.W))
