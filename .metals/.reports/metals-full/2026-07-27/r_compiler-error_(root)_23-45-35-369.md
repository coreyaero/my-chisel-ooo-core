...
### java.lang.AssertionError: assertion failed: file://<WORKSPACE>/src/main/scala/AguUnit.scala: 13420 >= 13392
...
class AguUnit extends Module {
    ...
    when(io.flush) {
        ...
        when(ex1_ready) {
            ...
            // 如果被阻塞了 (不 ready)，唯一要做的就是洗面具
            ex1_data.branch_mask := ex1_data.branch_mask & clear_mask
        }


        // --- EX1 寄存器流转 ---
        when(io.in.valid && ex1_allow_in) {
            ex1_valid := !is_killed(io.in.bits.branch_mask) // 如果进来的瞬间被杀，直接判无效
            ex1_data  := io.in.bits
            ex1_data.branch_mask := io.in.bits.branch_mask & clear_mask
        } .elsewhen(ex1_valid) {
            when(ex1_allow_in || is_killed(ex1_data.branch_mask)) {
                ex1_valid := false.B
            } .otherwise {
                ex1_data.branch_mask := ex1_data.branch_mask & clear_mask
            }
        ...
        when(ex1_real_valid && ex2_allow_in) {
            ...
            // 数据赋值
            ex2_state.data := ex1_data
            ex2_state.data.branch_mask := ex1_data.branch_mask & clear_mask
            
            // EX1 计算结果快照
            ex2_state.va         := va
            ex2_state.src2       := src2_fwd
            ex2_state.is_tlbsrch := is_tlbsrch
            ex2_state.dmw_hit    := dmw_hit
            ex2_state.dmw_pa     := dmw_pa
            ex2_state.dmw_mat    := dmw_mat
            
            // TLB 快照
            ex2_state.tlb_found  := io.tlb_port.found
            ex2_state.tlb_index  := io.tlb_port.index
            ex2_state.tlb_ppn    := io.tlb_port.ppn
            ex2_state.tlb_ps     := io.tlb_port.ps
            ex2_state.tlb_plv    := io.tlb_port.plv
            ex2_state.tlb_mat    := io.tlb_port.mat
            ex2_state.tlb_d      := io.tlb_port.d
            ex2_state.tlb_v      := io.tlb_port.v
            
            // MMU 快照
            ex2_state.crmd_pg    := io.mmu_config.crmd.pg
            ex2_state.crmd_da    := io.mmu_config.crmd.da
            ex2_state.crmd_plv   := io.mmu_config.crmd.plv
            ex2_state.crmd_datm  := io.mmu_config.crmd.datm
            
        } .elsewhen(ex2_valid) {
            when(io.out.ready || is_killed(ex2_state.data.branch_mask)) {
                ex2_valid := false.B
            } .otherwise {
                ex2_state.data.branch_mask := ex2_state.data.branch_mask & clear_mask
            }
        }
    }
