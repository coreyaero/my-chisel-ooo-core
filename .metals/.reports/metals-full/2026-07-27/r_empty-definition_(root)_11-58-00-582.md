error id: file://<WORKSPACE>/src/main/scala/Top.scala:disp_data
file://<WORKSPACE>/src/main/scala/Top.scala
empty definition using pc, found symbol in pc: disp_data
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -chisel3/iq/io/disp_data.
	 -chisel3/iq/io/disp_data#
	 -chisel3/iq/io/disp_data().
	 -chisel3/util/iq/io/disp_data.
	 -chisel3/util/iq/io/disp_data#
	 -chisel3/util/iq/io/disp_data().
	 -iq/io/disp_data.
	 -iq/io/disp_data#
	 -iq/io/disp_data().
	 -scala/Predef.iq.io.disp_data.
	 -scala/Predef.iq.io.disp_data#
	 -scala/Predef.iq.io.disp_data().
offset: 11899
...
class core_top extends RawModule {
    ...
    withClockAndReset(aclk, reset_high) {
        // ★ 新增：为 IC 和 DC 打造专属配置
        val icfg = CacheConfig(enablePrefetch = true)  // ICache 开启预取！
        val dcfg = CacheConfig(enablePrefetch = false) // DCache 绝对关闭预取！
        ...
        // ==========================================
        // ★ 新增：打平后的全局 Flush 仲裁中心
        // ==========================================
        val is_exception_flush = rob.io.wb_flush
        val is_branch_flush    = exec_engine.io.branch_req
        
        // 前端（IF/ID）的冲刷：异常或者分支预测失败都会触发
        val global_flush       = is_exception_flush || is_branch_flush
        
        // 跳转地址仲裁：异常处理入口优先级绝对高于分支预测恢复
        val flush_target_pc    = Mux(is_exception_flush, rob.io.wb_target_pc, exec_engine.io.branch_pc)
        // ==========================================
        ...
        // ---------------- 前端 IF -> ID (自适应双进单出缓冲) ----------------
        val fetch_buffer_reset = reset_high.asBool | global_flush
        ...
        // ---------------- ID -> Rename -> IQ/ROB ----------------
        val id_flush = reset_high.asBool || global_flush
        ...
        val dispatch_block = global_flush || is_mispredict || delayed_mispredict
        ...
        // 控制信号
        rename.io.flush      := is_exception_flush
