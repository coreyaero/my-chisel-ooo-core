...
// 专为 LSQ 到 DCache 定制的、带 Flush 保护的 Skid Buffer
class LsqDcacheSkidBuffer extends Module {
    val io = IO(new Bundle {
        val flush = Input(Bool())
        val enq   = Flipped(Decoupled(new @@LsqDcacheReq()))
        val deq   = Decoupled(new LsqDcacheReq())
    })

    // 深度为 2 的环形队列
    val q = Reg(Vec(2, new LsqDcacheReq()))
    val head  = RegInit(0.U(1.W))
    val tail  = RegInit(0.U(1.W))
    val count = RegInit(0.U(2.W)) // 0, 1, 2

    // 斩断一切组合逻辑！ready 仅取决于寄存器 count
    val space_available = (count < 2.U)
    io.enq.ready := space_available

    // valid 同样仅取决于寄存器 count
    val has_data = (count > 0.U)
    io.deq.valid := has_data
    io.deq.bits  := q(head)

    val do_enq = io.enq.valid && space_available
