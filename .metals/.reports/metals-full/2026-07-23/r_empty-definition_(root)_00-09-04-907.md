error id: file://<WORKSPACE>/src/main/scala/ExecutionEngine.scala:lsq_violation_pc
file://<WORKSPACE>/src/main/scala/ExecutionEngine.scala
empty definition using pc, found symbol in pc: lsq_violation_pc
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -chisel3/lsq/io/lsq_violation_pc.
	 -chisel3/lsq/io/lsq_violation_pc#
	 -chisel3/lsq/io/lsq_violation_pc().
	 -chisel3/util/lsq/io/lsq_violation_pc.
	 -chisel3/util/lsq/io/lsq_violation_pc#
	 -chisel3/util/lsq/io/lsq_violation_pc().
	 -lsq/io/lsq_violation_pc.
	 -lsq/io/lsq_violation_pc#
	 -lsq/io/lsq_violation_pc().
	 -scala/Predef.lsq.io.lsq_violation_pc.
	 -scala/Predef.lsq.io.lsq_violation_pc#
	 -scala/Predef.lsq.io.lsq_violation_pc().
offset: 10116
...
class ExecutionEngine extends Module {
    ...
    io.tlb_s1_vppn     := agu.io.tlb_s1_vppn
    io.tlb_s1_va_bit12 := agu.io.tlb_s1_va_bit12
    io.tlb_s1_asid     := agu.io.tlb_s1_asid
    agu.io.tlb_s1_found:= io.tlb_s1_found
    agu.io.tlb_s1_index:= io.tlb_s1_index
    agu.io.tlb_s1_ppn  := io.tlb_s1_ppn
    agu.io.tlb_s1_ps   := io.tlb_s1_ps
    agu.io.tlb_s1_plv  := io.tlb_s1_plv
    agu.io.tlb_s1_mat  := io.tlb_s1_mat
    agu.io.tlb_s1_d    := io.tlb_s1_d
    agu.io.tlb_s1_v    := io.tlb_s1_v
    ...
    io.cacop_en := agu.io.cacop_en
    io.cacop_op := agu.io.cacop_op
    io.cacop_is_icache := agu.io.cacop_is_icache
    ...
    // AGU 算出的物理地址、数据、异常直接喂给 LSQ
    // ★ 核心防抖：只有当 AGU 成功将 Store/TLB 广播上 CDB 时，LSQ 才进行登记，绝不重复入队！
    // ★ LSQ 随时倾听 AGU 的结果，只要 AGU 放行，LSQ 就记录地址
    lsq.io.agu_in_valid   := agu.io.out.valid && agu.io.out.ready
    lsq.io.agu_in_lsqIdx  := agu.io.out.bits.lsq_idx
    
    // ★ 核心修复：LSQ 需要物理地址，直接从 AGU 算好的访存专线里拿！
    // 彻底让 CDB 广播网 (ex_result) 和访存物理网 (data_sram.addr) 解耦！
    // ★ 核心修复：LSQ 需要物理地址，直接从 AGU 算好的访存专线里拿！
    // 彻底让 CDB 广播网 (ex_result) 和访存物理网 (data_sram.addr) 解耦！
    
    // ★ 终极异常防火墙：如果 AGU 报了异常（如预测执行引发的假 TLB Miss），
    // 传回来的 paddr 和 uncached 必定包含 X 态！必须强行清零，阻断 X 态流入 LSQ！
    val agu_has_exc = agu.io.out.bits.hasException
    
    lsq.io.agu_in_paddr   := Mux(agu_has_exc, 0.U(32.W), agu.io.data_sram.addr)
    lsq.io.agu_in_size    := Mux(agu_has_exc, 0.U(2.W),  agu.io.data_sram.size)
    lsq.io.agu_in_uncached:= Mux(agu_has_exc, false.B,   agu.io.data_uncached)
    lsq.io.agu_in_wdata   := agu.io.data_sram.wdata
    lsq.io.agu_in_wstrb   := agu.io.data_sram.wstrb
    lsq.io.agu_in_exc     := agu.io.out.bits.hasException
    lsq.io.agu_in_ecode   := agu.io.out.bits.ecode
    ...
    io.lsq_violation_pc    := lsq.io.lsq_v@@iolation_pc
